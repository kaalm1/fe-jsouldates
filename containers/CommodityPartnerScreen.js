...
class CommodityPartnerScreen extends React.Component {
  ...
  onPressNext = () => {
    if (this.state.commodityAnswers.length === this.props.commodityQuestions.length){
      const { navigate } = this.props.navigation;
      this.props.updateStoreMainInfo(this.state)
      AsyncStorage.getItem(Config.JWT).then((value)=>this.props.postUserInfo(value, {page: 'commodityAnswers'}))
...
const mapStateToProps = (state) => {
  return {
    commodityQuestions: state.signUp.info.commodityQuestions,
...
export default connect(mapStateToProps,mapDispatchToProps)(CommodityPartnerScreen);
